...
import { ArrowLeft, TriangleAlert, RotateCw } from 'lucide-react';
...
(detailsApiState!='404')?<div className='max-w-[1440px] mx-auto w-full flex gap-10 my-auto flex-col lg:flex-row'>
    ...
    <div className='md:py-6 flex flex-col gap-2'>
        <h2 className='text-[20px] md:text-[30px] font-bold'>
            {detailsApiState?.title}
        </h2>
        <p><b className='font-[650]'>- Til: </b>{detailsApiState?.language}</p>
        <p><b className='font-[650]'>- Davlat: </b>{detailsApiState?.country}</p>
        <p><b className='font-[650]'>- Yili: </b>{detailsApiState?.publishedAt}</p>
        <p><b className='font-[650]'>- Turi: </b>{detailsApiState?.resourceType}</p>
        <p><b className='font-[650]'>- Sahifalar: </b>{detailsApiState?.size} bet</p>
        <p className='lg:max-w-[400px]'><b className='font-[650]'>- Muallif(lar): </b>{detailsApiState?.authors?.join(", ")}</p>
        <p className='lg:max-w-[400px]'><b className='font-[650]'>- Tavsif: </b>{detailsApiState?.summary}</p>
...
<div className='dark:bg-[#ddd1] !w-full xl:max-w-[225px] xl:flex-col !rounded-[16px] xl:rounded-[5px] flex flex-row gap-4 overflow-x-scroll xl:overflow-x-hidden xl:overflow-y-scroll xl:max-h-[500px] px-5 py-4 bg-[#eee] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-[#0005] dark:[&::-webkit-scrollbar-thumb]:bg-[#fff5] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar]:bg-[#0001] [&::-webkit-scrollbar]:rounded-full xl:px-3 xl:py-2 [&::-webkit-scrollbar-thumb]:bg-[#0005] border-l-10 border-r-10 xl:border-5 xl:border-t-20 xl:border-b-20 dark:border-[#0001] border-[#eee]'>
    
    {
        ...
        othersApiState.map(el=>{
            return <Link to={`/details/${el?.id}`} className={'min-w-40 min-h-50 bg-[#ddd] rounded-[12px] bg-no-repeat bg-cover xl:rounded-[5px]'} style={{backgroundImage:`url(${el.cover})`}}>
            </Link>
        })
        :
